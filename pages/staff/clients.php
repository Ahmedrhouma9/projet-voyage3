...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    header('Location: /projet-voyage3/pages/staff/clients.php?deleted=1'); exit;
}
...
$clients = $pdo->query("SELECT u.*,COUNT(r.id) AS nb_res FROM users u LEFT JOIN reservations r ON r.user_id=u.id WHERE u.role='client' GROUP BY u.id ORDER BY u.created_at DESC")->fetchAll();
?>
<!DOCTYPE html><html lang="fr">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Clients — Staff | TunisieVoyages</title>
<link rel="stylesheet" href="/projet-voyage3/public/css/style.css"></head>
<body>
<nav class="navbar staff-nav"><div class="container nav-inner">
...
<div class="page-header"><h1>Gestion des Clients</h1><button class="btn-staff btn-sm" onclick="toggleForm()">+ Ajouter</button></div>
...
<div class="form-group"><label>Mot de passe <?= $editing?'(vide = inchangé)':'*' ?></label><input type="password" name="password" <?= $editing?'':'required' ?> minlength="6" placeholder="Min. 6 caractères"></div>
<div style="display:flex;gap:1rem;margin-top:.75rem">
<button type="submit" class="btn-staff"><?= $editing?'Enregistrer':'Ajouter' ?></button>
<a href="/projet-voyage3/pages/staff/clients.php" class="btn-secondary">Annuler</a>
</div></form></div>

<table class="data-table">
<thead><tr><th>Nom</th><th>Email</th><th>Téléphone</th><th>Réservations</th><th>Inscrit le</th><th>Actions</th></tr></thead>
<tbody>
<?php foreach ($clients as $c): ?>
<tr>
<td style="font-weight:600"><?= h($c['prenom'].' '.$c['nom']) ?></td>
<td><?= h($c['email']) ?></td><td><?= h($c['telephone']?:'—') ?></td><td><?= $c['nb_res'] ?></td>
<td><?= date('d/m/Y',strtotime($c['created_at'])) ?></td>
<td style="display:flex;gap:.4rem">
<a href="?edit=<?= $c['id'] ?>" class="btn-secondary btn-sm">Modifier</a>
<a href="?delete=<?= $c['id'] ?>" class="btn-danger btn-sm" onclick="return confirm('Supprimer ?')">Supprimer</a>
</td></tr>
<?php endforeach; ?>
<?php if(empty($clients)): ?><tr><td colspan="6" style="text-align:center;color:var(--text-muted);padding:2rem">Aucun client.</td></tr><?php endif; ?>
</tbody></table>
</main></div>
