...
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    $pdo->prepare("DELETE FROM users WHERE id=? AND role='staff'")->execute([(int)$_GET['delete']]);
    header('Location: /projet-voyage3/pages/admin/staffs.php?deleted=1'); exit;
}

$editing = null;
if (isset($_GET['edit']) && is_numeric($_GET['edit'])) {
    $s = $pdo->prepare("SELECT * FROM users WHERE id=? AND role='staff'");
    ...
    elseif (!filter_var($email,FILTER_VALIDATE_EMAIL)) { $msg='Email invalide.'; $type='error'; }
        ...
        if ($id) {
            if ($pwd && strlen($pwd)<6) { $msg='Mot de passe min. 6 caractères.'; $type='error'; }
                ...
                if ($pwd) $pdo->prepare("UPDATE users SET nom=?,prenom=?,email=?,telephone=?,mot_de_passe=? WHERE id=? AND role='staff'")->execute([$nom,$prenom,$email,$tel,password_hash($pwd,PASSWORD_BCRYPT),$id]);
                else $pdo->prepare("UPDATE users SET nom=?,prenom=?,email=?,telephone=? WHERE id=? AND role='staff'")->execute([$nom,$prenom,$email,$tel,$id]);
                $msg='Staff mis à jour.'; $type='success'; $editing=null;
            }
        } else {
            if (!$pwd||strlen($pwd)<6) { $msg='Mot de passe obligatoire (min. 6 car.).'; $type='error'; }
                ...
                else { $pdo->prepare("INSERT INTO users (nom,prenom,email,mot_de_passe,telephone,role) VALUES (?,?,?,?,?,'staff')")->execute([$nom,$prenom,$email,password_hash($pwd,PASSWORD_BCRYPT),$tel]); $msg='Staff créé.'; $type='success'; }
            }
        }
    }
}
...
<title>Staffs — Admin | TunisieVoyages</title>
...
<div class="page-header"><h1>Gestion des Staffs</h1><button class="btn-admin btn-sm" onclick="toggleForm()">+ Ajouter</button></div>
<?php if ($msg): ?><div class="alert alert-<?= $type==='error'?'error':'success' ?>"><?= h($msg) ?></div><?php endif; ?>
<?php if (isset($_GET['deleted'])): ?><div class="alert alert-success">Staff supprimé.</div><?php endif; ?>
...
<h2 style="font-size:1.05rem;font-weight:700;margin-bottom:1.25rem"><?= $editing?'Modifier le staff':'Créer un Staff' ?></h2>
<form method="POST">
...
<button type="submit" class="btn-admin"><?= $editing?'Enregistrer':'Créer' ?></button>
<a href="/projet-voyage3/pages/admin/staffs.php" class="btn-secondary">Annuler</a>
</div></form></div>

<table class="data-table">
<thead><tr><th>Nom</th><th>Email</th><th>Téléphone</th><th>Inscrit le</th><th>Actions</th></tr></thead>
<tbody>
<?php foreach ($staffs as $s): ?>
<tr>
<td style="font-weight:600"><?= h($s['prenom'].' '.$s['nom']) ?> <span class="badge badge-staff">Staff</span></td>
<td><?= h($s['email']) ?></td><td><?= h($s['telephone']?:'—') ?></td>
<td><?= date('d/m/Y',strtotime($s['created_at'])) ?></td>
<td style="display:flex;gap:.4rem">
<a href="?edit=<?= $s['id'] ?>" class="btn-secondary btn-sm">Modifier</a>
<a href="?delete=<?= $s['id'] ?>" class="btn-danger btn-sm" onclick="return confirm('Supprimer ce staff ?')">Supprimer</a>
</td></tr>
<?php endforeach; ?>
<?php if(empty($staffs)): ?><tr><td colspan="5" style="text-align:center;color:var(--text-muted);padding:2rem">Aucun staff.</td></tr><?php endif; ?>
</tbody></table>
</main></div>
